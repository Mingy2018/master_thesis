...
def voxel_encoder(inputs, phase_train=True, reuse=False):
    strides = [1, 2, 2, 2, 1]
...
def trainGAN(is_dummy=False, checkpoint=None, subcate=config.subcate):
    ...
    z_vector_v1 = voxel_encoder(vol_tensor_1, phase_train=True, reuse=False)
    ...
    vol_tensor_test = voxel_encoder(vol_tensor_2, phase_train=False, reuse=True)
    vol_tensor_test = tf.maximum(tf.minimum(vol_tensor_test, 0.99), -0.99)
    generated_vol_test = generator(vol_tensor_test, phase_train=False, reuse=True)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            _, reconstruction_loss = sess.run([optimizer_op_ae, recon_loss],feed_dict={vol_tensor_1: model_matrix.eval()})
            ...
            if ((epoch % 100 == 0) and (epoch <= 3200)) or ((epoch % 200 == 0) and (epoch > 3200)):
                ...
                g_objects.dump(train_sample_directory + str(epoch))
                id_ch = np.random.randint(0, batch_size, 4)

                for i in range(4):
                    if g_objects[id_ch[i]].max() > 0.5:
                        # save generated object as 'epoch_ID.binvox'
