...
def trainGAN(is_dummy=False, checkpoint=None, subcate=config.subcate):
    ...
    z_vector_v1, z_mu, z_log_var = voxel_var_encoder(pre_vol_tensor_1, phase_train=True, reuse=False)
    ...
    # KL-divergence
    kl_divergence_vol = -0.5 * tf.reduce_mean(tf.reduce_sum(1.0 + z_log_var - z_mu ** 2 - tf.exp(z_log_var),1))
    #print "The shape of kl_before", test1.shape
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            # load training data
            original_vol = sess.run(vol_tensor_1, feed_dict={batch_vox_1: dataset.modelpath2matrix(batch[0], padding = True)}) # original volumetric data
            ...
            latents = sess.run(z_vector_v1, feed_dict= {pre_vol_tensor_1: vol})
            print 'The max value of latent vector', latents.max()
            print 'The min value of latent vector', latents.min()
